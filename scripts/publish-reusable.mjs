// One command that takes an Obsidian note from the vault to the live site.
...
//
// It fills in the note's reusable-id when empty, imports the note and its attachments,
// validates every entry, then commits and pushes. Nothing is committed if validation fails.
...
if (!positional.length || has('help')) {
  ...
  --no-push        import and commit, but do not push
  --no-commit      import only; leave the changes in the working tree
...
const idOut = run('ensure ' + (config.idKey || 'reusable-id'),
  process.execPath, ['scripts/ensure-reusable-id.mjs', notePath, ...(has('dry-run') ? ['--dry-run'] : [])]);
...
if (has('no-commit')) { console.log('\nImported /reusable/' + id + '/. Changes left uncommitted.'); process.exit(0); }

// 4. commit only this entry's files, so unrelated work in the tree stays out of the commit
const paths = [path.join(config.entriesDir, id), config.mediaDir, path.join(config.entriesDir, 'README.md')];
run('git add', 'git', ['add', '--', ...paths.filter(fs.existsSync)]);
...
if (has('no-push')) { console.log('\nCommitted. Push skipped (--no-push).'); process.exit(0); }
